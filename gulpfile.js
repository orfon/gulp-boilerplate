...
/**
 * compiles the html as underscore template and copies the final files
 * the the distribution path.
 */
function html() {
   return gulp.src(paths.src.html)
      .pipe(template(require(paths.config)))
      .on("error", notifyError) // prevents gulp crashing on browserify errors
...
gulp.task("html", function() { return html() });
...
gulp.task("production", gulpsync.sync(["env:production", "clean", ["copy:images", "html", "sass", "compile"], "compress"]));
gulp.task("default", gulpsync.sync(["env:development", "clean", ["copy:images", "html", "sass", "compile", "templates"], "watch"]));